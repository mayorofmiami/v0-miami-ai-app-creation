...
-- ============================================
-- COMPOSITE INDEXES FOR council system
-- ============================================

-- Optimize council lookups by user and type
CREATE INDEX IF NOT EXISTS idx_councils_user_type_updated 
  ON councils(user_id, type, updated_at DESC);

-- Optimize council advisor queries
CREATE INDEX IF NOT EXISTS idx_council_advisors_council_position 
  ON council_advisors(council_id, position ASC);

-- Optimize debate queries by user
CREATE INDEX IF NOT EXISTS idx_council_debates_user_status_created 
  ON council_debates(user_id, status, created_at DESC);

-- Optimize debate response queries
CREATE INDEX IF NOT EXISTS idx_council_debate_responses_debate_round 
  ON council_debate_responses(debate_id, round_number ASC, created_at ASC);

-- Optimize prediction queries
CREATE INDEX IF NOT EXISTS idx_council_predictions_debate_outcome 
  ON council_predictions(debate_id, outcome, due_date ASC);

-- ============================================
-- COMPOSITE INDEXES FOR boardroom system
-- ============================================

-- Optimize board session queries
CREATE INDEX IF NOT EXISTS idx_board_sessions_user_created 
  ON board_sessions(user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_board_sessions_thread_created 
  ON board_sessions(thread_id, created_at DESC);

-- Optimize board response queries
CREATE INDEX IF NOT EXISTS idx_board_responses_session_round 
  ON board_responses(session_id, round_number ASC, created_at ASC);
