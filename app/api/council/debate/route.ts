        ...
        try {
          ...
          const advisorInfo = advisors.map((a) => ({
            ...
            icon: a.display_name.includes("Visionary")
              ? "🚀"
              : a.display_name.includes("Guardian")
                ? "🛡️"
          ...
          for (let round = 1; round <= 3; round++) {
            ...
            for (const advisor of advisors) {
              ...
              controller.enqueue(
                encoder.encode(
                  `data: ${JSON.stringify({ type: "advisor_start", advisor: advisor.display_name, round })}\n\n`,
                ),
              )
              ...
              controller.enqueue(
                encoder.encode(
                  `data: ${JSON.stringify({ type: "advisor_end", advisor: advisor.display_name, round })}\n\n`,
                ),
              )
            }
          ...
          const debateContext = allResponses
            .map((r) => `${r.advisor_name} (Round ${r.round_number}): ${r.content}`)
            .join("\n\n")
