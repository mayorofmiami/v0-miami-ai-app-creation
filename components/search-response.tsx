...
export const SearchResponse = memo(function SearchResponse({
  ...
  const sourceCount = useMemo(() => safeCitations.length, [safeCitations])
  ...
  const processTextWithCitations = useMemo(() => {
    return (text: string) => {
    ...
    <div className="w-full space-y-4 md:space-y-6 animate-in fade-in duration-500">
      ...
      <div className="prose prose-invert prose-miami max-w-none" role="article" aria-label="Search response">
        <ReactMarkdown
          ...
          components={{
            h1: ({ children }) => (
              <h1 className="text-2xl md:text-3xl font-bold text-foreground mb-4 md:mb-6 mt-6 md:mt-8 first:mt-0 text-balance leading-tight tracking-tight">
                {children}
              </h1>
            ),
            h2: ({ children }) => (
              <h2 className="text-xl md:text-2xl font-semibold text-foreground mb-3 md:mb-5 mt-5 md:mt-7 first:mt-0 text-balance leading-snug tracking-tight">
                {children}
              </h2>
            ),
            h3: ({ children }) => (
              <h3 className="text-lg md:text-xl font-semibold text-foreground mb-3 md:mb-4 mt-4 md:mt-6 first:mt-0 text-balance tracking-tight">
                {children}
              </h3>
            ),
            p: ({ children }) => (
            ...
            ul: ({ children }) => (
              <ul className="list-disc list-inside space-y-2 md:space-y-3 mb-4 md:mb-6 text-foreground">{children}</ul>
            ),
            ol: ({ children }) => (
              <ol className="list-decimal list-inside space-y-2 md:space-y-3 mb-4 md:mb-6 text-foreground">
                {children}
              </ol>
            ),
            ...
            code: ({ inline, children, ...props }: any) => {
              if (inline) {
                return (
                  <code className="px-1.5 py-0.5 rounded bg-miami-aqua/20 text-miami-aqua font-mono text-xs md:text-sm">
                    {children}
                  </code>
                )
              }
              return (
                <pre className="bg-muted/50 rounded-lg p-3 md:p-4 overflow-x-auto mb-3 md:mb-4 border border-miami-aqua/20">
                  <code className="text-xs md:text-sm font-mono text-foreground" {...props}>
                    {children}
                  </code>
                </pre>
              )
            ...
            blockquote: ({ children }) => (
              <blockquote className="border-l-4 border-miami-aqua/50 pl-3 md:pl-4 py-2 my-3 md:my-4 italic text-muted-foreground bg-miami-aqua/5 rounded-r">
                {children}
              </blockquote>
            ),
            a: ({ href, children }) => {
            ...
            cite: ({ node }: any) => {
              ...
              return (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block mx-0.5 px-1.5 text-[10px] md:text-[11px] font-medium rounded-full bg-miami-aqua/10 text-miami-aqua/90 hover:bg-miami-aqua/20 hover:text-miami-aqua transition-colors cursor-pointer no-underline whitespace-nowrap align-middle"
                  style={{
                    height: "18px",
                    lineHeight: "18px",
                    minHeight: "18px",
      ...
      {(actions || modelBadge || sourceCount > 0 || relatedButton) && (
        <div className="space-y-3 md:space-y-4">
          <div className="pt-3 md:pt-4">
            {modelBadge ? (
              ...
              <div className="border-t border-border/30 mb-3 md:mb-4" />
            )}

            <div className="flex items-center justify-between gap-1.5 md:gap-4">
              {/* Left: Action buttons */}
              <div className="flex items-center gap-1.5 md:gap-2 flex-shrink-0">{actions}</div>
              ...
              <div className="flex items-center gap-1.5 md:gap-2 flex-shrink-0 ml-auto">
                ...
                {sourceCount > 0 && (
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs md:text-sm font-medium text-muted-foreground hover:text-foreground transition-colors px-2 md:px-2.5 py-1.5 rounded-lg hover:bg-accent"
                    aria-expanded={isSourcesExpanded}
                    aria-label={`${sourceCount} sources available`}
                  >
                    ...
                    <span className="inline-flex items-center justify-center w-5 h-5 text-[10px] font-semibold rounded-full bg-miami-aqua/20 text-miami-aqua">
                      {sourceCount}
                    </span>
                    <svg
          ...
          {sourceCount > 0 && isSourcesExpanded && (
            <div id="sources-list" className="grid gap-2.5 animate-in fade-in slide-in-from-top-2 duration-300">
              {safeCitations.map((citation, index) => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group flex items-start gap-3 p-3 md:p-4 bg-muted/30 hover:bg-muted/50 rounded-lg border border-border hover:border-miami-aqua/50 transition-all duration-200 max-w-full overflow-hidden"
                >
                  <div className="flex-shrink-0 w-6 h-6 md:w-7 md:h-7 rounded-full bg-muted flex items-center justify-center text-muted-foreground font-medium text-xs md:text-sm group-hover:text-miami-aqua transition-colors border border-border">
                    {index + 1}
                  </div>
                  <div className="flex-1 min-w-0 overflow-hidden">
                    <div className="flex items-center gap-2">
                      <h4 className="font-medium text-xs md:text-sm text-foreground group-hover:text-miami-aqua transition-colors line-clamp-1">
                        {citation.title}
                      </h4>
                    ...
                    <p className="text-[10px] md:text-xs text-muted-foreground mt-1.5 line-clamp-2 leading-relaxed">
                      {citation.snippet}
                    </p>
                    <p className="text-[9px] md:text-[10px] text-muted-foreground/60 mt-1.5 truncate font-mono">
                      {citation.url}
                    </p>
                  </div>
                </a>
