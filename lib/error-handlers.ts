...
export type ErrorType = "rate_limit" | "ai_gateway_rate_limit" | "ai_gateway_error" | "config_error" | "generic"
...
export function handleSearchError({ error, user, status }: ErrorHandlerConfig): string {
...
export function handleImageError({ error, status }: { error: { message?: string }; status: number }): string {
  if (status === 429) {
